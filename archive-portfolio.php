 ...
 */
function emerson_portfolio_excerpt_length( $length ) {
    return 15;
}
add_filter( 'excerpt_length', 'emerson_portfolio_excerpt_length', 999 );
		...
		<?php $post_counter = 0; if ( have_posts() ) : ?>
			...
			<header class="page-header">
				<?php 
					...
					if ( get_theme_mod( 'emerson_portfolio_title' ) ) {
						printf( '<h1 class="page-title">%s</h1>', get_theme_mod( 'emerson_portfolio_title' ) );
					}

					if ( get_theme_mod( 'emerson_portfolio_text' ) ) {
						$description = wpautop( apply_filters( 'widget_text', get_theme_mod( 'emerson_portfolio_text' ) ) );
			...
			while ( have_posts() ) : the_post();
			    ...
			    switch ( get_theme_mod( 'emerson_portfolio_cols', '2' ) ) {
			        case '1':
			            $portfolio_class = '';
			            break;
			        case '2':
			    ...
			    $post_counter++;
			    $post_counter = ( $post_counter > get_theme_mod( 'emerson_portfolio_cols', '2' ) ) ? 1 : $post_counter;
	            ...
	            if ( ( get_theme_mod( 'emerson_portfolio_show_title', '1' ) ) )
					the_title( 	sprintf( '<h2 class="entry-title" itemprop="headline"><a href="%s" rel="bookmark">', 
									esc_url( get_permalink() ) ), 
								'</a></h2>' 
					);

				if ( ( get_theme_mod( 'emerson_portfolio_excerpt' ) ) )
					the_excerpt();
